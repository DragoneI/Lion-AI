...
async function generateEmbeddings(texts) {
  try {
    ...
    const res = await fetch(FASTAPI_URL + '/embed', {
      method: 'POST',
      headers,
      // user_id transmis pour que FastAPI indexe les vecteurs dans FAISS
      body: JSON.stringify({ texts, user_id: currentUser?.id || null })
    });
...
// ★ callAgent remplace searchDocuments : l'agent FastAPI classifie
// l'intention (search_documents / web_search / direct_answer) avec BGE
// (~5ms, pas d'appel LLM), exécute l'outil choisi, et renvoie le contexte.
// sendMessage() utilise tool_used pour décider si une recherche web est
// nécessaire — sans jamais faire les deux inutilement.
async function callAgent(query) {
  if (!currentUser) return { tool_used: 'direct_answer', context: [], confidence: 0 };
  try {
    const headers = await workerHeaders();
    const res = await fetch(FASTAPI_URL + '/agent', {
      method: 'POST',
      headers,
      body: JSON.stringify({ query })
    });
    if (!res.ok) return { tool_used: 'direct_answer', context: [], confidence: 0 };
    return await res.json();
  } catch (e) {
    console.warn('Agent call failed:', e.message);
    return { tool_used: 'direct_answer', context: [], confidence: 0 };
  }
}
...
function prepareCtx(userMsg, conv, searchResults, ragResults, convSummary) {
  ...
  if (searchResults?.length) {
    ...
    ctx.push({
      role: 'system',
      content: `RÉSULTATS DE RECHERCHE WEB (données réelles et à jour, obtenues à l'instant) :\n${facts}\n\nCes informations sont actuelles. Utilise-les directement pour répondre. Ne dis JAMAIS que tu n'as pas accès à internet ou aux informations en temps réel — tu les as, ci-dessus. Réponds en ${lang2}.`
    });
  }
  ...
  const hist = conv.contextMessages
    .filter(m =>
      m.role !== 'system' &&
      !m.content.startsWith('RÉSULTATS DE RECHERCHE WEB') &&
...
async function sendMessage() {
  ...
  // Agent classifie intention et exécute outil (FAISS → pgvector si search_documents).
  const agentResult = await callAgent(text);
  let ragResults = agentResult?.tool_used === 'search_documents' && agentResult.context?.length
    ? agentResult.context : null;
  // web_search si agent le recommande, ou si utilisateur le force.
  const shouldSearch = webSearchForced || agentResult?.tool_used === 'web_search';
